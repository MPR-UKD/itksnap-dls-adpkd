...
def config_hf_backend():
    ...
    if hasattr(hf, "configure_http_backend"):
        ...
        print(
            f"Configuring Huggingface HTTP client, verify={global_config.https_verify}"
        )
...
class nnInteractiveWrapper(ModelWrapper):
    ...
    def set_image(self, sitk_image):
        ...
        img = sitk.GetArrayFromImage(self.input_image)[
            None
        ]  # Ensure shape (1, x, y, z)
...
class SAM2Wrapper(ModelWrapper):
    ...
    def __init__(self, config: SegmentServerConfig = global_config):
        ...
        self.model = Sam2Model.from_pretrained(
            self.HF_REPO_ID, local_files_only=lfo
    ...
    def set_image(self, sitk_image: sitk.Image):
        ...
        if sitk_image.GetNumberOfComponentsPerPixel() == 3:
            self.image_arr = sitk.GetArrayFromImage(sitk_image)[
                None, :, :, :
            ]  # Add batch dimension
    ...
    def add_point_interaction(self, index_itk: list[int], include_interaction: bool):
        ...
        input_points = torch.tensor(
            [[[[index_itk[0], index_itk[1]]]]], dtype=torch.float32
        )
        ...
        self.all_points = (
            ...
            else torch.cat([self.all_points, input_points], dim=-2)
        )
        self.all_labels = (
            input_labels
            if self.all_labels is None
            else torch.cat([self.all_labels, input_labels], dim=-1)
        )

        # Prepare inputs
        print(
            f"Interactions {self.all_points.detach().cpu().numpy().squeeze()} with labels {self.all_labels.detach().cpu().numpy().squeeze()}"
        )
        inputs = self.processor(
            images=self.image_arr if self.image_embeddings_pt is None else None,
            original_sizes=self.image_sizes
            if self.image_embeddings_pt is not None
            else None,
        ...
        sitk.WriteImage(
            sitk.GetImageFromArray(outputs.pred_masks.squeeze().detach().cpu().numpy()),
            "/tmp/masksam.nii.gz",
        )
        with open("/tmp/sam_inputs.json", "wt") as f:
            json.dump(
                {
                    "points": self.all_points.detach().cpu().numpy().tolist(),
                    "labels": self.all_labels.detach().cpu().numpy().tolist(),
                },
                f,
                indent=2,
            )
...
class ADPKDWrapper(ModelWrapper):
    ...
    def set_image(self, sitk_image):
        ...
        # (minutes-long) segmentation right away so it runs while the user
        # prepares the first interaction.
    ...
    async def _wait_for_result(self) -> sitk.Image:
...
def get_model_listing():
    ...
    models = [nnInteractiveWrapper, SAM2Wrapper, ADPKDWrapper]
    model_list = []
    for model in models:
        model_info = {
            "id": model.ID,
...
def instantiate_model_wrapper(
    repo_id: str, config: SegmentServerConfig = global_config
) -> ModelWrapper:
    ...
    elif repo_id == ADPKDWrapper.ID:
        return ADPKDWrapper(config)
    else:
